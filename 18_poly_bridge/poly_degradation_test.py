#!/usr/bin/env python3
"""
Polynomial + Degradation Test: P(stable limit cycle | d) for 2D polynomial ODEs
with enforced degradation (negative linear self-terms).

Tests whether the exponential decay gamma ~ 0.20 comes from:
  (A) Hill-function saturation, or
  (B) Degradation-vs-activation architecture (negative linear self-terms opposing
      positive nonlinear feedback).

Model:
  dx/dt = -b1*x + [sum_{(i,j)!=(1,0)} a_{ij} * x^i * y^j] + a_{01}*y  -  eps * x * (x^2 + y^2)
  dy/dt = -b2*y + [sum_{(i,j)!=(0,1)} b_{ij} * x^i * y^j] + b_{10}*x  -  eps * y * (x^2 + y^2)

Identical to Study 18 (poly_bridge_scaling.py) except two coefficients are
sign-constrained: the x-equation's x-term and y-equation's y-term are forced
negative (degradation), drawn from -U[0.2, 2.0].

Parameters: d = (p+1)(p+2) total random coefficients (same as Study 18).
...
Result: P(limit cycle | d), fit gamma_poly_degrad from ln(1/P) ~ gamma*d
"""
...
# With degradation, P will likely be MUCH lower than Study 18's ~1.5%.
# Larger sample sizes needed to detect rare limit cycles.
D_SPECS = [
    (2, 12,    100_000),
    (3, 20,    200_000),
    (4, 30,    500_000),
...
GAMMA_CUSP = 0.197
GAMMA_HOPF = 0.175
GAMMA_POLY = 0.010  # Study 18 reference (flat, no decay)
...
def generate_poly_coefficients(p, N):
    ...
    DEGRADATION VARIANT: x-equation's x-term (1,0) and y-equation's
    y-term (0,1) are forced negative (degradation).
    ...
    # Enforce degradation: x-equation's x-term and y-equation's y-term
    # must be negative (degradation opposing activation).
    # Find indices of (1,0) and (0,1) in terms list.
    idx_x = None  # index of (1, 0) -- the x monomial
    idx_y = None  # index of (0, 1) -- the y monomial
    ...
    # Override: degradation terms (always negative)
    # Range [0.2, 2.0] matches Study 16 Hill model's b1, b2 range
    a_coeffs[:, idx_x] = -np.random.uniform(0.2, 2.0, N)  # -b1*x in x-equation
    b_coeffs[:, idx_y] = -np.random.uniform(0.2, 2.0, N)  # -b2*y in y-equation
...
def check_batch(p, N):
    ...
    Pipeline per batch:
    1. Generate random polynomial coefficients (with degradation override)
...
def main():
    ...
    pprint("  2D polynomial ODE with cubic damping + enforced degradation")
    pprint("=" * 70)
    pprint()
    pprint("Model:")
    pprint("  dx/dt = -b1*x + [other a_{ij} x^i y^j] - eps*x*(x^2+y^2)")
    pprint("  dy/dt = -b2*y + [other b_{ij} x^i y^j] - eps*y*(x^2+y^2)")
    pprint(f"  eps = {EPS_DAMP} (fixed cubic damping)")
    pprint(f"  b1, b2 ~ U[0.2, 2.0] (enforced degradation)")
    pprint(f"  d = (p+1)(p+2) total random parameters (same as Study 18)")
    pprint()
    pprint("Hypothesis: degradation-vs-activation architecture drives")
    pprint("  exponential decay of P(organized | d), not Hill saturation.")
    pprint()
    ...
    for p_deg, d, N in D_SPECS:
        pprint(f"--- d = {d} (degree {p_deg} polynomial + degradation), N = {N:,} ---")
        t0 = time.time()
    ...
    # Study 18 comparison (same d values, no degradation)
    pprint("Study 18 reference (no degradation):")
    s18_data = {12: 1.857e-02, 20: 2.079e-02, 30: 1.315e-02,
                42: 1.520e-02, 56: 1.310e-02}
    for i in range(len(d_vals)):
        d = d_vals[i]
        if d in s18_data:
            P_s18 = s18_data[d]
            ...
            if P_deg > 0:
                ratio = P_deg / P_s18
                pprint(f"  d={d}: P_degrad={P_deg:.3e}  P_poly={P_s18:.3e}  "
                       f"ratio={ratio:.4f}  ({ratio*100:.1f}%)")
            else:
                pprint(f"  d={d}: P_degrad=0  P_poly={P_s18:.3e}  "
                       f"ratio=0  (complete suppression)")
    pprint()

    # ================================================================
    #  PHASE 2: Fit gamma_poly_degrad, compare to all references
    # ================================================================
    pprint("=" * 70)
    ...
    for label, res, d_used, lnP_used in [
        ...
        if 'A' in res:
            ...
            ratio_poly = r['gamma'] / GAMMA_POLY if GAMMA_POLY > 0 else np.inf
            ...
            pprint(f"  gamma_Hopf = {GAMMA_HOPF:.6f}  ratio = {ratio_hopf:.3f} "
                   f"({diff_hopf:.1f}% diff)")
            pprint(f"  gamma_poly = {GAMMA_POLY:.6f}  ratio = {ratio_poly:.3f} "
                   f"(vs Study 18 no-degradation)")
            if diff_cusp < 20:
    ...
    if len(d_decay) >= 2:
        ...
        for i in range(len(d_decay) - 1):
            ...
            lp1, lp2 = lnP_decay[i], lnP_decay[i + 1]
    ...
    gamma_pd = res_decay['A']['gamma'] if 'A' in res_decay else None
    r2_pd = res_decay['A']['R2'] if 'A' in res_decay else None
    beta_pd = res_decay['C']['beta'] if 'C' in res_decay else None

    pprint(f"{'Source':<20} | {'gamma':>8} | {'R^2':>8} | {'Model class':<35}")
    pprint("-" * 80)
    pprint(f"{'gamma_cusp':<20} | {GAMMA_CUSP:8.4f} | {'0.995':>8} | "
           f"{'1D Hill (degradation + sat.)':<35}")
    pprint(f"{'gamma_Hopf':<20} | {GAMMA_HOPF:8.4f} | {'0.9997':>8} | "
           f"{'2D Hill (degradation + sat.)':<35}")
    pprint(f"{'gamma_poly':<20} | {GAMMA_POLY:8.4f} | {'0.498':>8} | "
           f"{'2D polynomial (no degradation)':<35}")
    if gamma_pd is not None:
        pprint(f"{'gamma_poly_degrad':<20} | {gamma_pd:8.4f} | {r2_pd:8.4f} | "
               f"{'2D polynomial + degradation':<35}")
    pprint()

    if beta_pd is not None:
        pprint(f"Stretched exponent beta_poly_degrad = {beta_pd:.4f}")
    ...
    if gamma_pd is not None:
        pprint(f"  gamma_poly_degrad = {gamma_pd:.4f}  (R^2 = {r2_pd:.4f})")
        pprint(f"  gamma_cusp        = {GAMMA_CUSP:.4f}")
        pprint(f"  gamma_Hopf        = {GAMMA_HOPF:.4f}")
        pprint(f"  gamma_poly        = {GAMMA_POLY:.4f}  (Study 18, no degradation)")
        pprint()

        if gamma_pd >= 0.15:
            pprint("  INTERPRETATION: DEGRADATION DRIVES DECAY")
            pprint(f"  gamma_poly_degrad = {gamma_pd:.4f} ~ gamma_cusp/Hopf ~ 0.18-0.20")
            pprint()
            pprint("  The architecture of biological regulation (degradation opposing")
            pprint("  activation) is what makes organized states exponentially rare")
            pprint("  with increasing parametric complexity. Hill-function saturation")
            pprint("  is not required -- the same exponential decay emerges from any")
            pprint("  system with degradation structure.")
            pprint()
            pprint("  Since ALL biological regulatory networks have degradation")
            pprint("  (proteins decay, populations die, concentrations dilute),")
            pprint("  the bridge applies to biology universally.")

        elif gamma_pd >= 0.05:
            pprint("  INTERPRETATION: DEGRADATION CONTRIBUTES BUT IS NOT SUFFICIENT")
            pprint(f"  gamma_poly_degrad = {gamma_pd:.4f} (partial recovery)")
            pprint(f"  0.05 <= gamma < 0.15: degradation creates some exponential")
            pprint(f"  decay but not the full gamma ~ 0.20. Saturation may contribute")
            pprint(f"  an additional factor.")
            pprint()
            pprint("  Follow-up: test polynomial + degradation + bounded activation")
            pprint("  (clamp polynomial terms to [-R, R]).")

        else:
            pprint("  INTERPRETATION: DEGRADATION ALONE DOES NOT DRIVE DECAY")
            pprint(f"  gamma_poly_degrad = {gamma_pd:.4f} (stays flat like Study 18)")
            pprint()
            pprint("  The exponential decay requires Hill-function saturation")
            pprint("  specifically, not just degradation structure. The architectural")
            pprint("  hypothesis is wrong -- something about bounded sigmoidal")
            pprint("  nonlinearities specifically creates the exponential rarity.")
    else:
        pprint("  INSUFFICIENT DATA for conclusion.")
        pprint("  Could not fit gamma_poly_degrad.")
