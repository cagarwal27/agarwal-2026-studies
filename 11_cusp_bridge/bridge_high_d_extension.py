...
X_SCAN = np.linspace(0.001, 5.0, 500)
...
def check_bistable_batch(k, N):
    ...
    while n_done < N:
        ...
        # Evaluate f(x) for all configs at all x-points: shape (bs, len(x))
        ...
        for ch in range(k):
            ...
            q = q_all[:, ch][:, None]
            h = h_all[:, ch][:, None]
            xq = np.exp(q * log_x[None, :])
            hq = np.exp(q * np.log(h))
            fv = fv + r * xq / (xq + hq)
        ...
        for idx in cand_idx:
            ...
            result = _verify_bistable(a_val, b_val, ch_list)
...
# Use a finer grid only for verification (brentq bracketing)
X_VERIFY = np.linspace(0.001, 5.0, 2000)

def _verify_bistable(a, b, channels):
    """
    Verify bistability by recomputing f(x) on a finer grid for root-finding.
    Refines roots with brentq, checks stability.
    Returns (x_eq, x_sad, lam_eq, dphi) or None.
    """
    fv = eval_f_vec(X_VERIFY, a, b, channels)
    sc_idx = np.where(np.diff(np.sign(fv)))[0]
    if len(sc_idx) < 3:
        return None
    ...
    for i in sc_idx[:6]:  # At most 6 sign changes
        try:
            r = brentq(lambda x: eval_f_scalar(x, a, b, channels),
                       X_VERIFY[i], X_VERIFY[i + 1], xtol=1e-10)
            roots.append(r)
        except:
            pass
